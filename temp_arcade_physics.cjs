...
const regex = /applyLocalPrediction\(input\) \{[\s\S]*?body\.setLinearVelocity\(new Vector3\(0, currentVel\.y, 0\)\);\n\s*\}/m;

const arcadePhysicsCode = `applyLocalPrediction(input) {
...
content = content.replace(regex, arcadePhysicsCode);

fs.writeFileSync(path, content, 'utf8');
console.log("Updated applyLocalPrediction with arcade driving proxy");
