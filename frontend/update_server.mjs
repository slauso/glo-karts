...
function updateFile(p) {
  ...
  file = file.replace(/this\.onMessage\("input",\s*\(client,\s*data\)\s*=>\s*\{[\s\S]*?\}\);/g,
  ...
  file = file.replace(/this\.state\.players\.forEach\(\(p,\s*id\)\s*=>\s*\{[\s\S]*?\}\);/g,
...
updateFile('C:/Users/laptop/twistedkart/realtime/src/rooms/RaceRoom.js');
updateFile('C:/Users/laptop/twistedkart/realtime/src/rooms/BattleRoom.js');
console.log('Finished updating backend rooms for Tweening and Coordinates');
