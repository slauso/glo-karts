/**
 * physics.js ΓÇö Backward-compatibility shim.
 *
...
// Module-level refs ΓÇö set by the game loop after kart loads
let _kartBody = null;
let _kartMesh = null;
...
/**
 * No-op ΓÇö physics is now initialised by babylon-renderer.js.
 * Kept for backward compatibility with existing callers.
 */
export async function initPhysics() {
  console.log('Physics world initialised (Havok ΓÇö unified scene)');
}
...
export function updatePhysics(deltaTime, carState, raceState) {
  ...
  if (!raceState.raceStarted || raceState.raceFinished) {
    // Just return zero speed ΓÇö kart is frozen (STATIC body)
    return { currentSpeed: 0, driftTier: 0, miniBoostTier: 0, miniBoostActive: false };
  }
