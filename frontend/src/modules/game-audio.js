...
function warnAudioOnce(url, reason) {
  if (_failedAudioUrls.has(url)) return;
  _failedAudioUrls.add(url);
  console.warn(`[game-audio] Audio fallback engaged for ${url}: ${reason}`);
}
...
async function loadBuffer(url) {
  ...
  if (_failedAudioUrls.has(url)) return null;

  try {
    const resp = await fetch(url);
    if (!resp.ok) {
      warnAudioOnce(url, `HTTP ${resp.status}`);
      return null;
    }

    // Vite/history fallback can return HTML with 200 for missing assets.
    const contentType = (resp.headers.get("content-type") || "").toLowerCase();
    if (contentType.includes("text/html")) {
      warnAudioOnce(url, "received HTML instead of audio");
      return null;
    }

    const arrayBuf = await resp.arrayBuffer();

    // Guard against empty or tiny non-audio payloads.
    if (!arrayBuf || arrayBuf.byteLength < 64) {
      warnAudioOnce(url, `invalid audio payload (${arrayBuf?.byteLength || 0} bytes)`);
      return null;
    }
    ...
    warnAudioOnce(url, e?.message || "decode failed");
    return null;
  }
}
