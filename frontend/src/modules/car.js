...
function loadCarModel(ammo, scene, carComponents, wheelPositions, onModelLoaded) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      // Find wheel meshes in the car model (with STK name fallbacks)
      const findWheel = (primary, ...alts) => {
        let m = carModel.getObjectByName(primary);
        for (let i = 0; !m && i < alts.length; i++) m = carModel.getObjectByName(alts[i]);
        return m;
      };
      let wheelMeshFL = findWheel('wheel-fr', 'wheel-front-right', 'WheelFR', 'wheel_fr');
      let wheelMeshFR = findWheel('wheel-fl', 'wheel-front-left', 'WheelFL', 'wheel_fl');
      let wheelMeshBL = findWheel('wheel-br', 'wheel-rear-right', 'WheelBR', 'wheel_br');
      let wheelMeshBR = findWheel('wheel-bl', 'wheel-rear-left', 'WheelBL', 'wheel_bl');
