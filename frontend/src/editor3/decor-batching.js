...
export function buildBatchedDecor(instances, opts = {}) {
  ...
  for (const [, bucket] of buckets) {
    ...
    // GLB-backed props can't share a single InstancedMesh (each clone owns
    // its own scene graph + materials), so we fall back to per-instance
    // visuals for them. They're rare in editor designs.
    if (def.glb) {
      for (const inst of bucket) {
        const m = buildDecorMesh(inst);
        if (m) { visuals.push(m); glbCount += 1; }
...
function addColliderForInstance(inst, localBox, chunks, chunkUnits, physicsMaterial) {
  ...
  //   - GLB props (localBox === null) → fall back to the old probe-Mesh
  //     path because GLB AABBs aren't trivially derivable.
  const def = DECOR[inst.type];
  if (!def) return;
  if (def.glb || !localBox) {
    if (def.glb) return; // GLB collider isn't generated by this batcher
