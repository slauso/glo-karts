...
(async () => {
    ...
    await page1.evaluate(() => {
        sessionStorage.setItem('gameConfig', JSON.stringify({
            ...
            multiplayer: true, roomName: 'gloflux', host: true
        }));
    ...
    await page2.evaluate(() => {
        sessionStorage.setItem('gameConfig', JSON.stringify({
            gameMode: 'gloflux', subMode: 'arena',
            multiplayer: true, roomName: 'gloflux', host: false
        }));
        sessionStorage.setItem('selectedKart', 'adiumy');
    });
