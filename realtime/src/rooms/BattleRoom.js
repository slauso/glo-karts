...
const BURST_QUEUE_LIMITS = {
  projectileFired: 192,
  projectileHit: 128,
  kartCrash: 64,
};
const BURST_FLUSH_BASE_BUDGETS = {
  projectileFired: 8,
  projectileHit: 6,
  kartCrash: 3,
};
...
export class BattleRoom extends Room {
    ...
    this.onMessage("fireWeapon", (client, data = {}) => {
        ...
        if (result.projectile) {
          ...
          if (ANOMALY_WEAPONS.has(proj.subType)) {
            ...
            this._queueBurstEvent("projectileFired", {
              id: proj.id,
  ...
  _queueBurstEvent(type, payload) {
    const queue = this._burstEventQueues?.[type];
    if (!queue) {
      this.broadcast(type, payload);
      return;
    }

    const limit = BURST_QUEUE_LIMITS[type] || 0;
    if (limit > 0 && queue.length >= limit) {
      queue.shift();
      const metrics = this.syncMetrics;
      if (metrics) {
        if (type === "projectileFired") metrics.battleDroppedProjectileFires += 1;
        else if (type === "projectileHit") metrics.battleDroppedProjectileHits += 1;
        else if (type === "kartCrash") metrics.battleDroppedKartCrashes += 1;
      }
    }

    queue.push(payload);
    this._updateBurstQueueMetrics();
  }

  _getBurstFlushBudget(type) {
    const queue = this._burstEventQueues?.[type];
    const backlog = queue?.length || 0;
    const base = BURST_FLUSH_BASE_BUDGETS[type] || 1;
    if (backlog <= base) return base;
    return Math.min(base + Math.ceil((backlog - base) / 12), base * 4);
  }

  _flushBurstEventQueues() {
    const queues = this._burstEventQueues;
    if (!queues) return;

    const flushType = (type, flushedMetricName) => {
      const queue = queues[type];
      const budget = this._getBurstFlushBudget(type);
      let flushed = 0;
      while (queue.length > 0 && flushed < budget) {
        this.broadcast(type, queue.shift());
        flushed += 1;
      }
      if (flushed > 0 && this.syncMetrics) {
        this.syncMetrics[flushedMetricName] += flushed;
      }
    };

    flushType("projectileFired", "battleFlushedProjectileFires");
    flushType("projectileHit", "battleFlushedProjectileHits");
    flushType("kartCrash", "battleFlushedKartCrashes");
    this._updateBurstQueueMetrics();
  }

  _updateBurstQueueMetrics() {
    const metrics = this.syncMetrics;
    if (!metrics || !this._burstEventQueues) return;
    metrics.battleQueuedProjectileFires = this._burstEventQueues.projectileFired.length;
    metrics.battleQueuedProjectileHits = this._burstEventQueues.projectileHit.length;
    metrics.battleQueuedKartCrashes = this._burstEventQueues.kartCrash.length;
  }
  ...
  _resolveCombatHit(hit = {}) {
    ...
    this._queueBurstEvent("projectileHit", {
      projectileId,
      victimId: victim.id,
      attackerId,
      subType,
  ...
  update(deltaTime) {
    ...
    for (const crash of kartCrashes) {
      ...
      this._queueBurstEvent("kartCrash", {
