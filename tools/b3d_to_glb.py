...
# ── GLB builder (mirrors spm_to_glb.py pattern) ───────────────────
...
def build_glb(all_meshes, tex_dirs, offset_map=None, part_names=None, scale_x=1.0):
    ...
    for part_idx, part_meshes in enumerate(all_meshes):
        ox, oy, oz = (0, 0, 0)
        if offset_map and part_idx in offset_map:
            ox, oy, oz = offset_map[part_idx]
        ...
        for local_idx, m in enumerate(part_meshes):
            ...
            if ox != 0 or oy != 0 or oz != 0:
                pos = [(x + ox, y + oy, z + oz) for x, y, z in pos]
    ...
    nodes = [{'mesh': i, 'name': gltf_meshes[i]['name']} for i in range(len(gltf_meshes))]
